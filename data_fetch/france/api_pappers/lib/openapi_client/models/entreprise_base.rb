  ...
  class EntrepriseBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OpenapiClient::EntrepriseBase` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OpenapiClient::EntrepriseBase`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [siren, siren_formate, nom_entreprise, personne_morale, denomination, nom, prenom, sexe, code_naf, libelle_code_naf, domaine_activite, conventions_collectives, date_creation, date_creation_formate, entreprise_cessee, date_cessation, entreprise_employeuse, categorie_juridique, forme_juridique, effectif, effectif_min, effectif_max, tranche_effectif, annee_effectif, capital, statut_rcs].hash
    end
